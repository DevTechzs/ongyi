    ...
    <script>
        ...
        $("#contactNo").on('change', function() {
            ...
            if (contactNo.length !== 10 || !/^\d+$/.test(contactNo)) {
                warningMessage("Please enter a valid 10-digit contact number.");
        ...
        function validateinputs() {
            ...
            let ifsc = $("#ifsc").val();
            let rrcFile = $('#rrc')[0].files;
            let licencefiles = $('#fssailicence')[0].files;
            // Check if all the required fields are filled
            if (contactNo === "" || accountNo === "" || bankName === "" || accountholder === "" || bankBranch === "" || ifsc === "" || rrcFile.length === 0 || licencefiles.length === 0) {
            ...
            if (accountNo.length !== 16 || !/^\d+$/.test(accountNo)) {
                warningMessage("Please enter a valid 16-digit account number.");
                return false;
            }
            // Check if the length of the IFSC is exactly 11 digits
            // if (ifsc.length !== 11 || !/^[A-Za-z]{2}[0-9]{2}[A-Za-z0-9]{3}$/.test(ifsc)) {
            //     warningMessage("Please enter a valid IFSC code.");
            //     return false;
            // }
            // File validation for RRC (only PDF files allowed)
            if (rrcFile.length === 0) {
                warningMessage("Please upload the required RRC document.");
                return false;
            } else if (rrcFile[0].type !== 'application/pdf') {
                warningMessage("RRC document must be in PDF format.");
                return false;
            }

            // File validation for FSSAI Licence (only PDF files allowed)
            if (licencefiles.length === 0) {
                warningMessage("Please upload the FSSAI licence document.");
                return false;
            } else if (licencefiles[0].type !== 'application/pdf') {
                warningMessage("FSSAI Licence document must be in PDF format.");
                return false;
            }
            return true;
        ...
        function onSuccess(rc) {
            ...
            if (rc.return_code) {
                switch (rc.Page_key) {
                    case "saveVendor":
                        showSuccessMessage(rc.return_data);
                        location.reload();
                        break;
                    default:
                        showErrorMessage(rc.Page_key);
                }
                rc.return_data
            } else {
                warningMessage(rc.return_data);
            }
        }
